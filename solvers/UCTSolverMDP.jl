...
function lookahead(𝒰, ℳ, s, a)
    S, A, T, R, H, V = ℳ.S, ℳ.A, ℳ.T, ℳ.R, ℳ.H, 𝒰.V
    T = T(ℳ,S[s],A[a])
...
function solve(𝒰::UCTSolver, 𝒱, ℳ)
    ...
    for r = 1:𝒰.max_rollouts_
        ...
        for i = 1:𝒰.cutoff_
            if node ∉ 𝒰.visited_
                ...
                for a = 1:length(A)
                    ...
                    node.count += 1
                    node.Q[a] = lookahead(𝒰, ℳ, node.state_id, a)
                end
            end
            ...
            reward = cum_reward[i] + R(ℳ, node.state, ℳ.A[a])
            push!(cum_reward, reward)
        ...
        for i = 1:maxSteps
            ...
            cum_reward_node = last(cum_reward) - cum_reward[i]
            δ_target = ((cum_reward_node - node.Q[a])
                       / node.action_count[a])
            node.Q[a] += δ_target
        end
        𝒰.V[node.state_id] = maximum(node.Q)
    end
